...
def evaluate_model(
    ...
    with torch.no_grad():
        for data, landslide in dataloader:
            ...
            targets = (landslide > 0.5).float()

            # Flatten: (B, C, H, W) -> (B*C*H*W). view(-1) "appiattisce" tutti i pixel di tutti i batch e canali
            # in un unico vettore per calcolare intersezione, unione e accuratezza come semplici somme.
            ...
            union = (
                ((preds_flat + targets_flat) > 0).float().sum().item()
            )  # ((preds_flat + targets_flat) > 0) restituisce un tensore booleano, quindi con .float() lo castiamo a valore numerico
            correct = (preds_flat == targets_flat).sum().item()
            total = preds_flat.numel()

            # Aggiorniamo i sommatori per calcolare su tutto il batch
            intersection_sum += intersection
            union_sum += union
            correct_sum += correct
            total_pixels += total

    avg_loss = total_loss_sum / max(1, num_samples)
    iou = intersection_sum / union_sum if union_sum > 0 else 1.0
    oa = correct_sum / max(1, total_pixels)
    return avg_loss, iou, oa
